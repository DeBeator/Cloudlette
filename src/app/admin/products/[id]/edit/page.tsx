...
import { getProductById, Product } from "@/lib/mock-data";
import { useToast } from "@/components/ui/toast";
import { FadeInSection } from "@/components/animations/FadeInSection";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { ArrowLeft, Plus, Trash2, UploadCloud, AlertTriangle } from "lucide-react";
...
export default function EditProductPage({
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setCategory(existing.category as "bag" | "shoe" | "top");
      setDescription(existing.description);
      setPrice(existing.price.toString());

      // Pre-fill variants
      const mapped = existing.variants.map((v, i) => ({
        id: v.id || `var-${i}`,
        color: v.color,
        size: v.size || "",
        stockQty: v.stock_qty ?? v.stockQty ?? 5,
      }));
      setVariants(mapped);
    }
  }, [params.id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    showToast(
      "Product Updated!",
      "Backend integration coming — changes saved in local session."
    );
    ...
    return (
      <FadeInSection className="max-w-4xl mx-auto px-4 py-20 text-center">
        <div className="bg-white rounded-3xl p-10 border border-blush/60 shadow-sm max-w-md mx-auto space-y-4">
          ...
          <h1 className="font-heading text-2xl font-normal text-dark">Product Not Found</h1>
          <p className="text-xs text-dark-muted font-light">
            The requested product ID could not be loaded for editing.
          </p>
          <Button asChild variant="goldOutline" className="rounded-full px-6 text-xs uppercase tracking-wider">
            <Link href="/admin/products" className="inline-flex items-center space-x-2">
            ...
            <div className="border-2 border-dashed border-blush rounded-3xl p-8 text-center bg-cream-light/30 hover:bg-cream-light/60 transition-colors cursor-pointer space-y-3">
              ...
              <div className="space-y-1">
                <p className="text-xs font-semibold text-dark">
                  Drag and drop updated product images here, or browse
                </p>
